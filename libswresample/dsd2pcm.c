...
{
    int t, e, m, sign;
    double acc[DSD_CTABLES];
    for (e = 0; e < 256; ++e) {
        memset(acc, 0, sizeof(acc));
        for (m = 0; m < 8; ++m) {
            sign = (((e >> (7 - m)) & 1) * 2 - 1);
            for (t = 0; t < DSD_CTABLES; ++t)
                acc[t] += sign * htaps[t * 8 + m];
        }
        for (t = 0; t < DSD_CTABLES; ++t)
            ctables[DSD_CTABLES - 1 - t][e] = acc[t];
    }
}
...
av_cold void swri_dsd2pcm_init(void)
{
    ...
    ff_thread_once(&init_static_once, dsd2pcm_ctables_tableinit);
}
    ...
    while (samples-- > 0) {
        ...
        *p = ff_reverse(*p);
        ...
        for (i = 0; i < DSD_CTABLES; i++) {
            uint8_t a = buf[(pos                   - i) & DSD_FIFOMASK];
            uint8_t b = buf[(pos - (DSD_CTABLES*2 - 1) + i) & DSD_FIFOMASK];
