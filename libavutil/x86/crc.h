...
static inline void crc_init_x86_avx512icl(AVCRC *ctx, int le, int bits, uint32_t poly, int ctx_size)
{
    uint64_t poly_;
    if (le) {
        // convert the reversed representation to regular form
...
{
    switch (ctx[0]) {
#if HAVE_CLMUL_EXTERNAL && !(defined(__AVX512F__) && defined(__VPCLMULQDQ__))
    ...
    case CLMUL_LE_AVX512ICL: return ff_crc_le_avx512icl(ctx, crc, buffer, length);
#endif
    default: av_unreachable("x86 CRC only uses CLMUL_BE, CLMUL_LE, CLMUL_BE_AVX512ICL and CLMUL_LE_AVX512ICL");
    }
    return 0;
}
